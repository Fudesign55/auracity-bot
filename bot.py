...
class DailyView(discord.ui.View):
    ...
    async def daily_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        daily_ch = get_setting(guild_id, "daily_channel_id")
        if daily_ch and str(interaction.channel.id) != str(daily_ch):
            return await interaction.response.send_message("❌ กดไม่ได้ที่ห้องนี้", ephemeral=True)

        ok, before, after, msg = claim_daily(guild_id, user_id)
        ...
        if ok:
            await send_log(
                interaction.guild,
                f"✅ DAILY <@{user_id}> | ก่อน: {before} | หลัง: {after}"
            )
